...
def test_parallel_cross_fit_matches_sequential():
    ...
    irm_seq = IRM(**kwargs, n_jobs=1).fit()
    irm_par = IRM(**kwargs, n_jobs=2).fit()
    ...
    est_seq = irm_seq.estimate(score="ATE", diagnostic_data=False)
    est_par = irm_par.estimate(score="ATE", diagnostic_data=False)
