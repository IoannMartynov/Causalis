...
def test_scm_fit_warnings_are_suppressed_and_exported_in_run_diagnostics(monkeypatch):
    ...
    exported_warnings = report["warnings"]
    assert any("SLSQP simplex optimization did not converge" in msg for msg in exported_warnings)
    assert any("Augmented donor weights are extreme" in msg for msg in exported_warnings)

    metrics = report["metrics"]
    assert metrics["slsqp_fallback_count"] >= 1
    assert any(
        "Positive directional derivative for linesearch" in reason
        for reason in metrics["slsqp_fallback_reasons"]
    )
