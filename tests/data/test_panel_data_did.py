            ...
            rows.append(
                {
                    ...
                    "treated_time": int(is_treated_unit and period >= pd.Period("2020-02", freq="M")),
                }
            )
...
def _canonical_2x2_df() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "unit_id": ["T1", "T1", "C1", "C1"],
            "time_id": pd.PeriodIndex(["2020-01", "2020-02", "2020-01", "2020-02"], freq="M"),
            "y": [10.0, 12.0, 8.0, 9.0],
...
def test_simultaneous_adoption_contract_and_helpers_work():
    ...
    assert did_df.loc[did_df["time_id"] < panel.treatment_start, "post"].eq(0).all()
    assert did_df.loc[did_df["time_id"] >= panel.treatment_start, "post"].eq(1).all()
...
def test_canonical_2x2_design_is_identified():
    ...
    assert set(counts["group"]) == {"treated", "control"}
...
def test_covariates_and_cluster_are_validated_and_projected():
    ...
    panel = PanelDataDID(
        **_base_kwargs(df=df, covariates=["x_num", "x_bool", "x_num"], cluster_col="cluster_id")
    )
    ...
    assert {"x_num", "x_bool", "cluster_id", "did_treated", "did_post"}.issubset(did_df.columns)


def test_covariants_and_cluster_aliases_are_accepted():
    df = _panel_df_with_covariates_cluster()

    panel = PanelDataDID(**_base_kwargs(df=df, covariants="x_num", cluster="cluster_id"))
...
def test_covariates_and_cluster_must_be_distinct_from_disallowed_roles():
    ...
    with pytest.raises(ValueError, match="cluster_col must be distinct from y and treated_time"):
        PanelDataDID(**_base_kwargs(df=df, cluster_col="treated_time"))

    with pytest.raises(ValueError, match="cluster_col must be distinct from covariates"):
...
def test_covariates_must_exist_be_numeric_non_null_and_non_constant():
    ...
    with pytest.raises(ValueError, match="Covariate 'x_num' contains non-numeric values"):
...
def test_requires_at_least_one_treated_row_and_one_control_unit():
    ...
    with pytest.raises(ValueError, match="Need at least one never-treated control unit"):
        PanelDataDID(**_base_kwargs(df=df_no_control))


def test_staggered_adoption_is_rejected():
    df = _base_panel_df()
    df.loc[(df["unit_id"] == "T2") & (df["time_id"] == pd.Period("2020-02", freq="M")), "treated_time"] = 0

    with pytest.raises(ValueError, match="simultaneous adoption"):
        PanelDataDID(**_base_kwargs(df=df))


def test_treated_units_must_stay_treated_after_start():
    df = _base_panel_df()
    df.loc[(df["unit_id"] == "T1") & (df["time_id"] == pd.Period("2020-03", freq="M")), "treated_time"] = 0

    with pytest.raises(ValueError, match="for treated_units must be 1 at/after treatment_start"):
        PanelDataDID(**_base_kwargs(df=df))


def test_analysis_periods_must_have_treated_and_control_rows():
    df = _base_panel_df()
    df = df[~((df["unit_id"] == "C1") & (df["time_id"] == pd.Period("2020-03", freq="M")))].copy()
    df = df[~((df["unit_id"] == "C2") & (df["time_id"] == pd.Period("2020-03", freq="M")))].copy()

    with pytest.raises(ValueError, match="Each analysis period must contain"):
        PanelDataDID(**_base_kwargs(df=df))
...
def test_panel_repr_is_compact_and_informative():
    ...
    assert "control_units=['C1', 'C2']" in repr_str
